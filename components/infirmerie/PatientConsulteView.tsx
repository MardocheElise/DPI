// route: components/infirmerie/patients-consultes-view.tsx
"use client"
...
import { formatDateHeureFr, PatientConsulte, PATIENTS_CONSULTES } from "@/lib/dataInfirmerie/infirmerie"
...
export function PatientsConsultesView() {
  ...
  const [sortKey, setSortKey] = useState<SortKey | null>("dateConsultation")
  ...
  const rows = useMemo(() => {
    ...
    if (filters.dateDebut) {
      data = data.filter((p) => p.dateConsultation >= filters.dateDebut)
    }
    if (filters.dateFin) {
      data = data.filter((p) => p.dateConsultation <= `${filters.dateFin}T23:59:59`)
    }
  ...
  return (
    <div className="space-y-6">
      ...
      <p className="text-2xl font-semibold text-slate-700">
        <span className="text-[#0d9488]">{rows.length}</span>{" "}
        <span className="text-xl font-normal">patients déjà consultés</span>
      </p>
        ...
        <Table>
          <TableHeader>
            <TableRow className="bg-slate-50/60 hover:bg-slate-50/60">
              ...
              <SortableTh label="Consulté le" sortKey="dateConsultation" activeKey={sortKey} direction={direction} onSort={handleSort} />
              <SortableTh label="Praticien" sortKey="praticien" activeKey={sortKey} direction={direction} onSort={handleSort} />
          ...
          <TableBody>
            {rows.length === 0 ? (
              ...
              rows.map((p) => (
                <TableRow
                  key={p.id}
                  onClick={() => router.push(`/infirmerie/consultation/${p.id}`)} // TODO: route détail consultation
                ...
                >
                  ...
                  <TableCell onClick={(e) => e.stopPropagation()}>
                    <div className="flex items-center justify-end gap-2">
                      <Button
                        size="sm"
                        onClick={() => router.push(`/infirmerie/consultation/${p.id}`)} // TODO
                      ...
                      <Button
                        size="sm"
                        variant="outline"
                        onClick={() => router.push(`/infirmerie/consultation/${p.id}/fiche`)} // TODO
                        className="gap-2 rounded-lg border-[#0CB2DA]/40 font-semibold text-[#0CB2DA] hover:bg-[#0CB2DA]/10 hover:text-[#0CB2DA]"
                      >
