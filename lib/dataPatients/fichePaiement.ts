// Route: /accueil/paiements (données statiques en attendant la connexion BD)
// Type FichePaiement + jeu de données de démonstration.
// Une fiche correspond à un patient enregistré pour lequel une fiche de paiement a été créée.

export type StatutFiche = "payee" | "impayee" | "partiel";

export interface FichePaiement {
  id: number;
  code: string; // ex: FICHE-2026-WO53076-771157
  patient: string; // ex: CISSE Aw1
  patientCode: string; // ex: D26E7943BBB
  service: string; // ex: MEDECINE GENERALE
  coutTotal: number; // en FCFA
  coutAssure: number; // pris en charge
  netAPayer: number; // reste à payer
  statut: StatutFiche; // état du règlement
